"""
BKZ (fpylll) baseline for DNS (Defect-Noise Scenario).

BKZ is a stronger lattice reduction than LLL (higher block_size = stronger but slower).
Infeasibility detection: heuristic — "no solution found" → declared infeasible.
As with LLL, this is inexact: hard feasible instances may still return None.
BKZ's higher reduction quality means fewer false negatives than LLL.
"""
import json
import time
import numpy as np
...
def run(data_dir=DEFAULT_DATA_DIR, block_size=DEFAULT_BLOCK_SIZE):
    data = load_data(data_dir)
    total = len(data)
    if total == 0:
        raise SystemExit(f"No JSON files found in {data_dir}")

    print(f"\n[BKZ (fpylll, block_size={block_size})] {total} instances  |  dir: {data_dir}")
    print("  Infeasibility detection: heuristic (no solution found → declared infeasible)")
    print(f"  Block size tip: 2=LLL, 10=fast, 20=balanced, 40=strong, 60+=very slow")
    ...
    for idx, (A, b, _, gt_feasible) in enumerate(data):
        ...
        pred_feasible, pred_sol = solve(A, b, block_size)
    ...
    gt_i = tn + fp
    print_summary(
        f"BKZ (fpylll, block_size={block_size})",
        total, gt_f, gt_i, tp, tn, fp, fn, sol_correct, times,
...
if __name__ == "__main__":
    ...
    p.add_argument("--block_size", type=int, default=DEFAULT_BLOCK_SIZE,
                   help="BKZ block size (default: 20)")
    args = p.parse_args()
    run(args.data_dir, args.block_size)
