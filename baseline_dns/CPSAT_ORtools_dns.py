"""
Google OR-Tools CP-SAT baseline for DNS (Defect-Noise Scenario).
Exact solver: INFEASIBLE status → 100% reliable feasibility detection.
"""
import json
import time
import numpy as np
...
try:
    ...
    raise SystemExit("ortools not found. Install with: pip install ortools")
...
def solve(A, b, time_limit_sec=60.0):
    """
    Returns (pred_feasible: bool, pred_solution: ndarray | None).
    OPTIMAL/FEASIBLE → feasible, solution returned.
    INFEASIBLE       → infeasible, None returned.
    UNKNOWN          → conservative: declare feasible, no solution.
    """
    m, n = A.shape
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        return True, None  # UNKNOWN / time limit → conservative
...
def run(data_dir=DEFAULT_DATA_DIR, time_limit=60.0):
    ...
    print(f"\n[CP-SAT (OR-Tools)] {total} instances  |  dir: {data_dir}")
    ...
    gt_f = tp + fn
    gt_i = tn + fp
    print_summary("CP-SAT (OR-Tools)", total, gt_f, gt_i, tp, tn, fp, fn, sol_correct, times)
