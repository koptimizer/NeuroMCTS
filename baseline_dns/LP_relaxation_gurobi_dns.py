"""
Gurobi LP Relaxation baseline for DNS (Defect-Noise Scenario).

Infeasibility detection capability:
  - LP INFEASIBLE  → binary system is definitely infeasible (LP ⊇ binary feasible set)
  - LP OPTIMAL     → binary system MAY be feasible; round and verify
                     If rounded x does not satisfy Ax=b, declare feasible-but-unsolved
                     (LP cannot prove binary infeasibility in this case)

This means LP will miss infeasible cases where the LP relaxation still has a
fractional feasible solution — which is the typical case when b is perturbed
within [0, row_max(A)] bounds.  Expect partial infeasibility detection.
"""
import json
import time
import numpy as np
...
def solve(A, b, env):
    ...
    LP OPTIMAL, Ax̂=b    → (True,  x̂)       x̂ = round(x_lp)
    LP OPTIMAL, Ax̂≠b    → (True,  None)     LP can't prove infeasibility here
    """
    m, n = A.shape
    model = gp.Model(env=env)
    x = model.addMVar(shape=n, lb=0.0, ub=1.0, vtype=GRB.CONTINUOUS, name="x")
    model.addMConstr(A.astype(float), x, "=", b.astype(float))
    ...
    if model.Status == GRB.INFEASIBLE:
        ...
        x_round = np.round(x.X).astype(np.int8)
        if np.all(A.dot(x_round.astype(np.int32)) == b):
            return True, x_round
        else:
            return True, None   # LP feasible but no integer solution found
    else:
        return True, None
...
def run(data_dir=DEFAULT_DATA_DIR):
    ...
    print(f"\n[Gurobi LP Relaxation] {total} instances  |  dir: {data_dir}")
    ...
    for idx, (A, b, _, gt_feasible) in enumerate(data):
        ...
        pred_feasible, pred_sol = solve(A, b, env)
    ...
    gt_i = tn + fp
    print_summary(
        "Gurobi LP Relaxation", total, gt_f, gt_i, tp, tn, fp, fn, sol_correct, times,
