...
We also report what the method does not achieve. CP-SAT solves the same instances $41$--$62\times$ faster at every size tested, and our system does not compete with it; decomposing that gap shows our branching produces $1.6\times$ \emph{fewer} nodes but processes them $78\times$ more slowly, locating the deficit in a research implementation rather than in search quality. And the cost advantage over probing, while asymptotically real, is only $1.3$--$1.7\times$ at the depths where prediction matters most on our instances, where probing additionally offers proofs rather than predictions. The contribution we claim is therefore methodological: exact conditional posteriors are an obtainable and well-posed supervision signal for branching, in-tree states are a better training distribution than root states, and the resulting heuristic transfers across instance size. Code, generators, exact-marginal labels and per-instance results are released.
\end{abstract}
...
The learned guidance produces a \emph{smaller} search tree than CP-SAT --- $1.6\times$ fewer nodes despite CP-SAT's clause learning --- and loses entirely on per-node cost. That cost is an artifact of our implementation rather than of the method: our loop is Python, it rebuilds and re-solves the node relaxation from scratch rather than warm starting from the parent basis, and it materializes a reduced instance at every node instead of maintaining incremental data structures. Section~\ref{sec:exp-cost} measures the same effect directly: switching to a warm-started backend cuts our per-node LP cost several-fold. We did not port the search loop itself, so the throughput column reflects research code, not a limit of the approach.
