...
\item \textbf{End-to-end evaluation under matched wall-clock budgets.} At $21\times60$, replicated over three seeds on a warm-started search loop, the learned guidance is $2.6$--$3.0\times$ faster in median than LP-based branching (faster on $23/30$ instances in every seed, sign test $p=0.005$) and expands $9.5\times$ fewer nodes; both strategies solve $30/30$ within $600$\,s (Section~\ref{sec:exp-search}). In the deployed cascade both sound strategies close every residual instance, the learned one $3.0\times$ faster ($40/50$ wins pooled over seeds, $p=2\times10^{-5}$), while random branching closes none of the $50$ residual instances (Section~\ref{sec:exp-cascade}).
...
\bottomrule
\end{tabular}
\caption{Median solve time; all methods solve $30/30$ at every size. The proposed column is the \textsc{model} arm of Table~\ref{tab:search} (mean of per-seed medians). The proposed system does not beat a complete solver at any size tested, and is outperformed by SCIP, whose LP code it uses for training features.}
\label{tab:classical}
\end{table}
...
$21\times60$ & \textsc{model} & 10--16/30 & 50/50 & \textbf{14.2--20.2\,s} & $100\pm0.0\%$ \\
\bottomrule
\end{tabular}}
...
\paragraph{The advantage is in time and tree size, not in solve rate.} On a warm-started loop both sound strategies solve every instance we evaluate within budget; what the learned guidance buys at $21\times60$ is a $2.6$--$3.0\times$ median speedup and $9.5\times$ fewer nodes, significant per instance in every seed. An earlier draft reported a $5$--$0$ solve-rate advantage from a loop that rebuilt the relaxation at every node; that difference vanished with the faster backend and we do not claim it. Whether a solve-rate advantage exists at larger budgets or sizes is untested, and with $30$ evaluation instances per size the power to detect one would be low in any case.
...
All generators are seeded. The released checkpoints are $96$\,KB each. Instance data and weights are regenerated from the above rather than distributed, and the full evaluation cycle is scripted in \texttt{v23\_ablation/\allowbreak run\_revision2.sh}.
