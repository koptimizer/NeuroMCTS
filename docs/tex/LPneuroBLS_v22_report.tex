...
\title{Learning as Amortized Deduction:\\
Ceilings, Efficiency, and Size Transfer in Binary Linear Systems}
...
\begin{abstract}
Binary linear systems (BLS) --- find $\mathbf{x}\in\{0,1\}^n$ with $A\mathbf{x}=\mathbf{b}$ for $A\in\{0,1\}^{m\times n}$ --- arise in laser-based non-destructive testing, where a binary image must be recovered from integer projections. Deciding feasibility is NP-complete, and on the market-split-style family studied here classical solvers dominate every learned method this project has tried. Across eleven cycles, twelve learned components were tested and eleven produced null results, which had been read as evidence that the input carries no learnable signal. That reading was never tested; this report tests it and shows it is wrong. Because the generator plants one solution $\mathbf{x}^*$ and publishes $\mathbf{b}=A\mathbf{x}^*$, every member of the solution set $\mathcal{S}$ would have produced the same $\mathbf{b}$; the posterior is therefore uniform on $\mathcal{S}$ and the exact per-variable marginal $p_j=|\{\mathbf{x}\in\mathcal{S}:x_j=1\}|/|\mathcal{S}|$ can be computed by exhaustive enumeration. The marginals partition the free variables of any node into \textsc{forced} ones, where every surviving solution agrees and branching the other way empties the subtree, and \textsc{free} ones, where solutions disagree and either branch still reaches a solution. Only \textsc{forced} errors cost anything, and this partition --- not the aggregate accuracy it is usually summarized by --- is what connects prediction to search. Four findings follow. First, the partition explains eleven cycles of null results. At the root $93.7\%$ of variables are \textsc{free}, so the $69.9\%$ Bayes ceiling on recovering $\mathbf{x}^\ast$ is $94\%$ composed of predictions that cannot cost a backtrack; every prior cycle trained against $\mathbf{x}^\ast$ and was therefore optimizing a target that mostly does not matter. A small single-head GNN comes within $1.9$ points of that ceiling while the project's previous multi-head network sat at the level of plain LP rounding ($60.7\%$ vs.\ $60.6\%$), so the nulls reflect a misdirected objective and an ill-suited model rather than absent information. Second, descending the tree inverts the ratio --- \textsc{forced} rises from $6.3\%$ at the root to $89.0\%$ at depth $8$ --- which is where accuracy starts translating into search cost; holding $n$ fixed and varying $m$ confirms there is no operating point where learning wins by a wide margin, since where the ceiling is low it is already saturated, where it is high attaining it is NP-hard, and higher still the LP relaxation suffices. Third, conditioning on a partial assignment is identical to reducing the instance, so the same architecture predicts \emph{inside} the search tree; the conditional ceiling rises from $70.5\%$ at the root to $93.4\%$ at depth $8$, and decomposing the residual shows it lies entirely on variables the prefix logically determines, $92$--$99\%$ of which unit propagation fails to detect. LP-probing recovers $62$--$79\%$ of them but costs one LP solve per variable; the network answers every variable in one forward pass at $1.6$\,ms, roughly $20\times$ cheaper, with equal or better hit rate. Learning's role is therefore \emph{amortization of expensive deduction}, not discovery of inaccessible information --- an instance of amortized optimization \citep{amos2023tutorial}, and the same role branching imitation plays in mixed-integer programming \citep{gasse2019exact}. Fourth, used as branching guidance this yields a median $2.71\times$ speedup over LP-based guidance at $21\times60$ (sign test $p=0.019$), a $6.2\times$ reduction in nodes, and a solve-rate increase from $90.0\%$ to $100\%$ within a 600\,s budget; in the deployed cascade, where lattice reduction runs first, coverage of that stage falls from $30/30$ at $n=25$ to $16/30$ at $n=60$ and guidance lifts the end-to-end rate from $93.3\%$ to $100\%$. Controlling $|\mathcal{S}|$ across sizes --- previously confounded by a factor of ten --- removes an artifact that had been misread as transfer failure: a model trained only at $10\times25$ is statistically indistinguishable from one trained at the target size when both are evaluated at $21\times60$ ($15/30$ instances faster, total time differing by $0.4\%$). All instance generators, exact-marginal labels, checkpoints and per-instance results are released with the code.
\end{abstract}
...
That interpretation was never tested. It is a claim about the information content of the input, and for this instance family that content is measurable \emph{exactly} --- not estimated, not bounded from below by whatever a particular model happens to achieve:
...
\label{eq:marginal}
\end{equation}
which is right a fraction $\max(p_j,1-p_j)$ of the time. Enumerating $\mathcal{S}$ therefore gives an exact upper bound on how well \emph{any} predictor can recover $\mathbf{x}^\ast$ from $(A,\mathbf{b})$.
\end{quote}

Enumeration is feasible at $n\le 60$ with a constraint solver, which makes the ceiling an experimental quantity rather than a theoretical one.

Two things this bound is not, stated here because the distinction governs how every later result should be read. It is not a bound on \emph{solving}: the solver's task is to return some member of $\mathcal{S}$, not to identify which member was planted, and the two diverge --- an unconstrained instance has $\mathcal{S}=\{0,1\}^n$, hence the worst possible ceiling of $50\%$, while being trivial to solve. Nor is it a proxy for instance difficulty: across our solved $21\times60$ instances, $|\mathcal{S}|$ and search time have Spearman $\rho=-0.04$, so the two axes are decoupled rather than inverted.

What the bound does settle is narrower and, for this project, decisive. Every previous cycle trained against $\mathbf{x}^\ast$ as the per-variable label, so~\eqref{eq:marginal} is exactly the target those cycles were pursuing, and measuring it separates ``the information is absent'' from ``the information is present and the model failed to extract it'' --- an attribution that eleven null results had left open. It also induces a partition of the variables at each search node, into those the current prefix logically determines and those it does not, and that partition is what carries the operational content into search (\S\ref{sec:m-ceiling}). Everything in this report follows from being able to measure it.

\subsection{Contributions}

\begin{enumerate}
\item \textbf{A partition that says which predictions matter, and where they are.} Exact marginals split each node's free variables into \textsc{forced} (all surviving solutions agree; branching against it empties the subtree) and \textsc{free} (they disagree; either branch reaches a solution). At the root only $6.3\%$ are \textsc{forced}, so root-level accuracy is $94\%$ made of decisions that cannot cost a backtrack --- which is precisely the target all eleven prior cycles were trained on, since they used $\mathbf{x}^\ast$ as the per-variable label. By depth $8$ the ratio inverts to $89.0\%$ \textsc{forced} (Sections~\ref{sec:ceiling},~\ref{sec:depth}). The useful information is not absent; it sits at a depth nobody had looked at.

\item \textbf{The ceiling as a diagnostic and a controlled variable.} On the target the prior cycles actually pursued, the Bayes-optimal accuracy is $69.9\%$; a small single-head network reaches $68.6\%$ while the previous multi-head network reached $60.7\%$, indistinguishable from LP rounding at $60.6\%$ --- the nulls reflect a misdirected objective and an ill-suited model, not absent signal. Holding $n$ fixed and raising $m$ lifts the ceiling from $70.5\%$ to $100\%$ and exposes three regimes, none of which learning wins by a wide margin: where it is low it is already attained; where it is high, attaining it is NP-hard; higher still, the LP relaxation suffices (Section~\ref{sec:sweep}).
...
\citet{bengio2021machine} survey this area and frame it in terms this report inherits: state-of-the-art solvers ``rely on handcrafted heuristics for making decisions that are otherwise too expensive to compute or mathematically not well defined,'' and learning is a candidate for making those decisions better. The phrase \emph{too expensive to compute} is the operative one. It presumes the decision is computable in principle and asks only what it costs --- which is precisely the situation Section~\ref{sec:cost} measures, and a different situation from asking a network to supply information the input does not contain.
...
\subsection{Amortization: the framing this report adopts}
\label{sec:amort}

The distinction that organizes our results predates its use in combinatorial optimization. \citet{gershman2014amortized} introduced \emph{amortized inference} for probabilistic reasoning: when an agent must answer many related queries, solving each from scratch is wasteful, and the useful question becomes how to reuse computation across queries. \citet{amos2023tutorial} develops the same idea as \emph{amortized optimization} --- ``use learning to predict the solutions to problems in these settings, exploiting the shared structure between similar problem instances'' --- and surveys its appearance in variational inference, meta-learning, control and convex optimization.

\citet{millidge2022deconfusing} states the trade-off in the form most useful here. \emph{Direct} optimization spends computation on the instance at hand, so its power scales with the compute it is given but it pays that cost at every invocation. \emph{Amortized} optimization converts the problem into supervised learning: inference is a single cheap forward pass, but it requires an expensive dataset of solved instances up front and is bounded by the function approximator's generalization. The two are complementary rather than competing, and the productive arrangement is hybrid --- amortized predictions steering a direct optimizer, as in AlphaGo.

Every element of that account has a measured counterpart below, which is why we adopt the framing rather than merely borrowing the word. The direct method here is LP-probing, sound and costly at one LP solve per variable (Table~\ref{tab:cost}). The amortized method is MarginalNet, a single forward pass roughly $20\times$ cheaper. The expensive dataset requirement appears as our hardest limitation: exact marginal labels need exhaustive enumeration of $\mathcal{S}$, which fails beyond $n\approx60$ (\S\ref{sec:instances}). The generalization bound appears as the $18$--$20$ point residual in the near-unique regime (\S\ref{sec:sweep}). And the hybrid arrangement is exactly our design rule: the network \emph{guides} branching, where an error costs backtracking, while variables are \emph{fixed} only by sound deduction, where an error would cost correctness (\S\ref{sec:m-search}).

What this report adds to that literature is the missing denominator. Amortization is normally justified by showing the learned predictor is fast and accurate enough in practice; it is rarely possible to say how much of the attainable signal it captures, because the attainable signal is unknown. For planted-solution instances it is computable, and computing it changes the interpretation of failure: a null result can be attributed to an absent ceiling rather than to an inadequate model, or --- as happened repeatedly in this project --- the reverse.
...
\paragraph{What this ceiling does and does not bound.} It is worth stating precisely, because the quantity is easy to over-read. Equation~\eqref{eq:ceiling} bounds \emph{per-variable identification of the planted $\mathbf{x}^\ast$}. It is not a bound on the task the solver actually performs, which is to return \emph{any} member of $\mathcal{S}$. The two come apart, and in the obvious direction: an instance with no constraints at all has $\mathcal{S}=\{0,1\}^n$, hence $p_j=\tfrac12$ everywhere and a ceiling of exactly $50\%$ --- the worst possible --- while being trivially solvable. A low ceiling therefore does not mean a hard instance; it means the planted solution is not recoverable, which is a different statement.

Our data show the two axes are decoupled rather than inverted. Across the $27$ solved $21\times60$ instances, $|\mathcal{S}|$ (range $2$--$40$) and search time have Spearman $\rho=-0.04$ ($t=-0.18$, not significant), so within the range we can measure, solution multiplicity predicts search difficulty neither way.

Why measure it, then? Two reasons, and both are about diagnosis rather than difficulty. First, every previous cycle in this project trained against $\mathbf{x}^\ast$ as the per-variable label, so~\eqref{eq:ceiling} is exactly the bound those training setups faced --- and finding the prior network at $60.7\%$ against a $69.9\%$ ceiling and a $60.6\%$ LP baseline diagnoses the setup, not the problem. Second, the decomposition the ceiling induces does carry operational meaning inside the tree, through the following correspondence.

\paragraph{The bridge from marginals to search.} At a node, a free variable is \textsc{forced} ($p_j\in\{0,1\}$: every surviving solution agrees) or \textsc{free} ($0<p_j<1$: they disagree). Branching on a \textsc{free} variable cannot go wrong in the sense that matters --- both children contain solutions, so either choice keeps a solution reachable. Branching \emph{against} a \textsc{forced} variable empties the subtree and guarantees a backtrack. The marginal is thus read not as ``probability of matching $\mathbf{x}^\ast$'' but as ``fraction of $\mathcal{S}$ retained by this assignment,'' and only errors on \textsc{forced} variables cost anything.

This is why Section~\ref{sec:cost} decomposes the residual gap by \textsc{forced}/\textsc{free} rather than reporting an aggregate: the aggregate mixes decisions that can cost a backtrack with decisions that cannot. It is also why a model scoring near $50\%$ on \textsc{free} variables (Table~\ref{tab:gap}) is behaving correctly rather than failing, and why the operationally meaningful quantity is in-tree \textsc{forced} accuracy, not root-level accuracy against $\mathbf{x}^\ast$.
...
\subsection{What the ceiling is made of, and how close models come to it}
\label{sec:ceiling}
...
\paragraph{Most of that number is made of decisions that cannot cost anything.} Splitting the same $600$ root variables by the criterion of \S\ref{sec:m-ceiling} gives Table~\ref{tab:rootsplit}, and it reframes the headline figure.

\begin{table}[h]
\centering
\begin{tabular}{lrrr}
\toprule
Root variables & count & share & ceiling on this subset \\
\midrule
\textsc{forced} (branching against it empties the subtree) & 38 & 6.3\% & 100.0\% \\
\textsc{free} (either branch still reaches a solution) & 562 & 93.7\% & 67.2\% \\
\midrule
all variables (the $69.9\%$ headline) & 600 & 100\% & 69.3\% \\
\bottomrule
\end{tabular}
\caption{Root-level decomposition, $24$ instances of $10\times25$ with $\mathcal{S}$ enumerated. The aggregate ceiling is $94\%$ composed of \textsc{free} variables, on which no prediction can cost a backtrack.}
\label{tab:rootsplit}
\end{table}

The $69.9\%$ is therefore not a statement about how hard these instances are to solve. It is a statement about how hard the planted solution is to identify, and $94\%$ of it is carried by variables where identification is irrelevant because both values lead to a solution. On the $6.3\%$ that could matter, the ceiling is already $100\%$ --- those variables are logically determined, and the question is not whether the information exists but whether anything detects it, which Section~\ref{sec:cost} takes up.

Three readings follow. (i) \emph{The nulls were not caused by absent information}: $69.9\%$ is well above chance, and on the subset that matters the ceiling is perfect. What was absent was a reason for root-level accuracy to help. (ii) \emph{A small dedicated model nearly exhausts it}: MarginalNet is within $1.9$ points overall and $5.7$ on top-3, while the prior multi-head network is statistically indistinguishable from LP rounding ($60.7\%$ vs.\ $60.6\%$) --- it had learned little beyond copying its LP input feature. This reframes several past nulls as model and task mismatch rather than absent signal. (iii) \emph{The label-noise hypothesis is only partly right}: soft labels clearly improve posterior estimation ($L_1$ $0.1261\to0.0977$, $-22\%$) but leave classification accuracy unchanged, so noisy labels were not the principal cause of past failures.
...
The ceiling rises steeply --- $70.5\%\to93.4\%$ by depth $8$ --- while propagation still forces only $8.2\%$ of remaining variables. The more consequential movement is in the composition rather than the level: the \textsc{forced} share rises from $6.3\%$ at the root (Table~\ref{tab:rootsplit}) to $89.0\%$ at depth $8$ (Table~\ref{tab:gap}). At the root almost every prediction is unfalsifiable in the only sense that matters, because both branches reach solutions; by depth $8$ almost every prediction is one that empties a subtree when wrong. This inversion, not the rising ceiling, is why in-tree prediction is the place where accuracy converts into search cost. Training on in-tree states beats the root-only model by $5.3$--$11.5$ points over depths $6$--$10$, confirming that in-tree learning is worthwhile in itself.
...
\paragraph{Against the amortization literature's predictions.} Read through \citet{millidge2022deconfusing}'s account of the trade-off (\S\ref{sec:amort}), our results land where that account says they should, including in the costs. The predicted benefit --- inference reduced to one cheap forward pass --- is realized at $1.6$\,ms against $31$--$37$\,ms, and the predicted hybrid advantage is realized in the cascade, where guidance lifts the end-to-end rate from $93.3\%$ to $100\%$ without ever being trusted to fix a variable. The predicted costs are equally visible. The requirement for an expensive corpus of solved instances is our binding limitation, since exact marginals demand exhaustive enumeration and enumeration fails beyond $n\approx60$. The bound imposed by the approximator's generalization is the $18$--$20$ point residual at $m=14$--$16$, where the ceiling is high but attaining it is NP-hard.

One result is not anticipated by that framing and is worth stating separately. Amortization is usually argued for on efficiency grounds while accuracy is assumed to degrade relative to the expert being imitated; here the amortized predictor is \emph{both} cheaper and more accurate than LP-probing on the \textsc{forced} set ($88$--$91\%$ against $61$--$80\%$ detection, Table~\ref{tab:cost}). The reason is that the two answer different questions: probing abstains wherever its test does not fire, whereas the network commits everywhere. For guidance, where abstention has no value because some variable must be branched on, committing everywhere at moderate accuracy dominates proving a subset exactly. The relevant comparison is therefore not accuracy against the expert but accuracy against \emph{what the consumer of the prediction can use}, and that consumer is the branching rule.

\subsection{Limitations}

\paragraph{The ceiling is low, and we lowered it.} Maximizing vertex spread is what makes these instances hard for CP-SAT, and it is also what drives the root ceiling to $69.9\%$ and the root \textsc{forced} share to $6.3\%$. An easier family would offer more signal, and more of it would be consequential, but it would not be the target regime.
...
Three directions follow from the measurements rather than from speculation. (i) \emph{Approximate marginals beyond enumeration}: if $p_j$ could be estimated by sampling (e.g.\ solution sampling or belief propagation) with usable fidelity, training data could be generated where enumeration fails. This is the dataset-cost term that \citet{millidge2022deconfusing} identifies as amortization's characteristic price, and relaxing it is the single change that would most extend the method's reach. (ii) \emph{Learning to imitate LP-probing directly}: Table~\ref{tab:cost} shows the network already matches probing's hit rate at $1/20$ the cost without being trained to imitate it; explicit distillation from probing decisions is the natural next step, and is precisely the learn2branch recipe. (iii) \emph{Replacing the pipeline's propagator}: the measurement that unit propagation detects under $8\%$ of forced variables while LP-probing detects $62$--$79\%$ is an improvement available immediately and independent of learning.

\section{Conclusion}
\label{sec:conclusion}

Eleven cycles of null results on binary linear systems had been read as evidence that the input carries no learnable signal. Because these instances have planted solutions, that reading was testable, and it was wrong --- though not in the way we first expected. Enumerating the solution set gives the exact posterior, and the posterior partitions each node's free variables into those the current prefix logically determines, where branching the wrong way empties the subtree, and those it leaves open, where either branch still reaches a solution. Only the first kind can cost anything.

At the root the second kind accounts for $93.7\%$ of variables. The Bayes ceiling there is $69.9\%$, but $94\%$ of that number is carried by predictions that cannot cause a backtrack, and on the $6.3\%$ that can, the ceiling is already $100\%$. Every prior cycle trained against the planted $\mathbf{x}^\ast$ and was therefore optimizing the part that does not matter. That --- rather than absent information --- is the common cause behind the nulls; a small single-head network comes within $1.9$ points of the ceiling while the project's previous multi-head network performed at the level of LP rounding, so an ill-suited model compounded a misdirected objective.

The useful information is not absent; it sits at a depth nobody had looked at. Descending the tree inverts the ratio --- the consequential variables rise from $6.3\%$ of the total at the root to $89.0\%$ by depth $8$ --- and that is where accuracy begins converting into search cost. Turning the ceiling into a controlled variable shows the signal is nonetheless hard to profit from: where it is low it is already attained, where it is high attaining it is NP-hard, and higher still the LP relaxation suffices. The productive question is therefore not how much a network knows but how cheaply it knows it. At depth $8$ the residual lies entirely on the forced variables, and classical LP-probing recovers most of them at one LP solve per variable while the network answers all variables in a single $1.6$\,ms pass --- a $19$--$23\times$ cost advantage at equal or better accuracy. Used as branching guidance this gives a $2.71\times$ median speedup at $21\times60$ ($p=0.019$), $6.2\times$ fewer nodes, and a solve rate of $100\%$ against $90.0\%$; in the deployed cascade, where lattice reduction absorbs a share of instances that falls from $30/30$ at $n=25$ to $16/30$ at $n=60$, guidance lifts the end-to-end rate from $93.3\%$ to $100\%$. With solution multiplicity matched across sizes, a model trained only at $10\times25$ is statistically indistinguishable from one trained at the target size, and an earlier ``transfer failure'' is explained as an ablation artifact.
