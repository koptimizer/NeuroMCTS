...
class MCTS:
    def __init__(self, model, device, num_simulations=50, c_puct=1.5, training=True, gumbel_initial_sims=10):
        self.model = model
        self.device = device
        self.num_simulations = num_simulations
        self.c_puct = c_puct
        self.training = training
        self.gumbel_initial_sims = gumbel_initial_sims
        self.cache = {}
...
class Trainer:
    def __init__(self, args):
        ...
        now = datetime.now()
        self.save_dir = Path(self.args.save_dir) / (now.strftime("%y%m%d-%H%M") + "_LP_Hybrid_v4")
    ...
    def _run_mcts_phase(self, num_epochs, is_il_phase):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for b_idx in pbar:
                ...
                if collected_trajectories:
                    ...
                    for chunk_start in range(0, len(collected_trajectories), NETWORK_BATCH_SIZE):
                        ...
                        select_log_probs_flat = select_log_probs.view(-1)
                        index_in_batch = [si * num_vars + item['target_var'] for si, item in enumerate(chunk)]
                        selected_var_log_probs = select_log_probs_flat[torch.tensor(index_in_batch, dtype=torch.long, device=self.device)]
                        selected_action_log_probs = F.log_softmax(relevant_assign_logits, dim=1)
                        chosen_action_idx = torch.tensor([int(np.argmax(act)) for act in list_act_target], dtype=torch.long, device=self.device)
                        act_log_prob = selected_action_log_probs[torch.arange(batch_size, device=self.device), chosen_action_idx]
                        raw_var_loss = -selected_var_log_probs
                        raw_act_loss = -act_log_prob
                        v_loss = F.mse_loss(v_pred.view(-1), b_val_target)
                        dist = Categorical(logits=relevant_assign_logits)
                        ent = dist.entropy().mean()
                        with torch.no_grad():
                            adv = b_val_target - v_pred.view(-1).detach()
                            adv_std = adv.std(correction=0)
                            adv_norm = (adv - adv.mean()) / (adv_std + 1e-8) if adv_std > 1e-6 else torch.zeros_like(adv)
                        p_loss = ((raw_var_loss + raw_act_loss) * adv_norm).mean()
            ...
            # ── Epoch summary ────────────────────────────────────────────────────
            feas_update_denom = max(1, sum(1 for _ in range(num_batches)))  # rough count
            if valid_update_count > 0:
