...
#!/usr/bin/env python3
"""
Inference harness for LPneuroBLS v5 (DNS — Defect-Noise Scenario).
...
def policy_dfs(A, b, root_assignment, var_order, val_order, node_budget=20000):
    # Complete backtracking search from the propagated root, with variable and
    # value ordering taken from the GNN policy; exhausting the tree without a
    # solution is a proof of infeasibility, exhausting the budget is 'unknown'
    nodes = 0
    ...
    while stack:
        ...
        if nodes > node_budget:
            return None, 'budget'
        env = m.FastBinaryEnv(A, b, assignment.copy())
        env.propagate_constraints()
        if env.is_invalid:
            continue
        if env.is_terminal():
            if env.get_accuracy() == 1.0:
                return env.assignment.copy(), 'solved'
            continue
...
def kernel_pump(x0, A, b, A_pinv, max_iter=200, seed=0):
    # Alternate rounding with projection onto the affine subspace {x : Ax = b};
    # the projection step moves along the null space of A, so every projected
    # point satisfies Ax = b exactly and only integrality remains to be fixed
    ...
    for _ in range(max_iter):
        ...
        if np.array_equal(Af.dot(x_r), bf):
            return x_r.astype(np.int8), True
...
def infer_instance(model, mcts, A, b, x_lp_var, x_lp_check, x_lp, A_pinv, device,
                   ...
                   lp_is_feasible=True, pump_iters=200, dfs_budget=20000):
    """
    Returns
    -------
    ...
    x_pump, pumped = kernel_pump(x_lp, A, b, A_pinv, max_iter=pump_iters)
    if pumped:
    ...
    with torch.no_grad():
        ...
        select_logits, assign_logits, _, feas_logit = model(xv, xc, x_lp_var, x_lp_check, ev2c, ec2v)
    ...
    x_pump, pumped = kernel_pump(x0, A, b, A_pinv, max_iter=pump_iters, seed=1)
    if pumped:
    ...
    sol, status = policy_dfs(A, b, root_assign, var_order, val_order, node_budget=dfs_budget)
    if status == 'solved':
...
def print_summary(checkpoint, total, gt_f, gt_i,
    ...
    w = 66
    print("\n" + "=" * w)
    print(f"Result Summary: LPneuroBLS v5")
    ...
    for name in ['propagation_infeasible', 'lp_hard_rule', 'kernel_pump',
...
def main():
    ...
    parser.add_argument('--out_dir',      type=str,   default='./inference_out_v5')
