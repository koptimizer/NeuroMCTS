...
class BipartiteGNN(nn.Module):
    ...
    def forward(self, x_var, x_check, x_lp_var, x_lp_check, edge_v2c, edge_c2v, batch_v=None, batch_c=None, pump_feats=None):
...
class Trainer:
    def __init__(self, args):
        ...
        now = datetime.now()
        self.save_dir = Path(self.args.save_dir) / (now.strftime("%y%m%d-%H%M") + "_LP_Hybrid_v6")
    ...
    def featurize(self, A, b, x_gt, gt_feasible):
        ...
        return {
            ...
            'xlp_v': torch.tensor(x_lp, dtype=torch.float, device=self.device),
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for cs in range(0, len(data), BATCH):
                ...
                for item in chunk:
                    ...
                    batch_input.append((xv, xc, item['xlp_v'], item['xlp_c'], item['ev2c'], item['ec2v'], None, None))
                bxv, bxc, bxlpv, bxlpc, bev2c, bec2v, _, _, bidxv, bidxc = self.batch_converter(batch_input)
                ...
                head_opt.zero_grad()
                _, _, _, feas_logits = self.model(bxv, bxc, bxlpv, bxlpc, bev2c, bec2v, bidxv, bidxc, pump_feats=b_pump)
    ...
    def pretrain(self):
        ...
        for epoch in range(1, self.args.pretrain_epochs + 1):
            ...
            for b_idx in pbar:
                ...
                _, assign_logits, _, feas_logits = self.model(bx_v, bx_c, b_xlpv, b_xlpc, bev2c, bec2v, b_idx_v, b_idx_c, pump_feats=b_pump)
                if b_msk.sum() > 0:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for b_idx in pbar:
                ...
                for item in current_batch_data:
                    ...
                    with torch.no_grad():
                        xv_root, xc_root = clean_env.get_tensor_state(self.device)
                        _, _, _, feas_logit_root = self.model(
                            xv_root, xc_root, x_lp_var, x_lp_check, edge_v2c, edge_c2v,
                            pump_feats=item['pump'].unsqueeze(0)
                        )
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for item in pbar:
                ...
                select_logits, assign_logits, _, _ = self.model(
                    xv, xc, item['xlp_v'], item['xlp_c'], item['ev2c'], item['ec2v']
                )
