...
#!/usr/bin/env python3
"""
Inference harness for LPneuroBLS v6 (DNS — Defect-Noise Scenario).
...
def infer_instance(model, mcts, A, b, x_lp_var, x_lp_check, x_lp, A_pinv, device,
    ...
    with torch.no_grad():
        ...
        select_logits, assign_logits, _, feas_logit = model(xv, xc, x_lp_var, x_lp_check, ev2c, ec2v, pump_feats=pump_ts)
...
def print_summary(checkpoint, total, gt_f, gt_i,
    ...
    w = 66
    print("\n" + "=" * w)
    print(f"Result Summary: LPneuroBLS v6")
...
def main():
    ...
    parser.add_argument('--out_dir',      type=str,   default='./inference_out_v6')
