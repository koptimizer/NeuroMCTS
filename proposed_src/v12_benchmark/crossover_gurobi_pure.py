"""Pure Gurobi MILP baseline with honest tri-state reporting (mirrors crossover_cpsat_pure.py) --
for a fair MILP vs CP-SAT comparison under the identical instance sets and time cap used in the
crossover experiment. TIME_LIMIT hit is reported as unresolved, not guessed as feasible."""
import argparse
import json
import time
import numpy as np
...
def solve(A, b, env, time_limit_sec):
	m, n = A.shape
	model = gp.Model("bls", env=env)
	...
	if model.Status == GRB.OPTIMAL:
		sol = np.round(x.X).astype(np.int64)
		return 'feasible', sol
...
def run(data_dir, time_limit, out_json):
	...
	for name, A, b, gt_feasible in data:
		...
		status, sol = solve(A, b, env, time_limit)
	...
	w = 62
	print("\n" + "=" * w)
	print(f"Result Summary: Gurobi MILP pure (honest unresolved), cap={time_limit}s")
