...
P('방법은 두 관찰에 기반한다. <b>첫째</b>, BLS를 partial assignment로 conditioning하는 것은 그 배정을 '
  ...
  'depth 8에서는 그 비중이 87.9%가 되며, 정확도가 탐색 비용으로 환산되기 시작하는 지점이 바로 여기다.', abs_s)
P('우리는 21.8K 파라미터의 bipartite graph network를 이 target에 학습시키고, 오직 branching에만 사용한다 — '
  ...
  'warm start한 탐색 루프에서 21×60 기준 LP 기반 branching 대비 median <b>2.6~3.0배</b> 빠르고(모든 seed에서 30개 중 23개에서 우세, '
...
P('<b>(2) 비용을 발생시킬 수 있는가에 따른 branching 결정의 분할.</b> 정확한 marginal이 node의 자유 변수를 '
  ...
  '<b>87.9%</b>다(§4.3, §4.4). 이는 쓸모 있는 예측이 root가 아니라 <b>tree 내부</b>에 있음을 특정하며, '
...
H2('4.6 탐색 성능')
P('lattice 단계를 끄고 branching 품질만 격리한 채, wall-clock 예산을 맞춰 branching 전략을 비교한다. '
  'lp는 가장 정수에 가까운 relaxation 값을 갖는 변수를 그 값으로 먼저 분기하고, model은 제안 network를 쓴다. 둘은 같은 루프에서 '
  '돈다 — node당 warm start relaxation 한 번(§4.2), 동일한 propagation, 동일한 값 순서 — 따라서 두 arm은 어느 변수로 분기하고 '
  '어느 값을 먼저 시도하는지만 다르다. 각 설정을 seed를 바꿔 세 번 실행하며, depth-first search 자체는 relaxation vertex가 '
  '주어지면 결정적이므로 seed는 시간만 바꾸고 node 수는 바꾸지 않는다.', body_s)
TBL([['크기 (예산)', '전략', '해결율', 'node 중앙값', '시간 중앙값', 'node/s'],
     ['10×25 (60초)', 'lp', '100±0.0%', '46', '<b>0.03±0.00초</b>', '1,767'],
     ['', 'model', '100±0.0%', '<b>24</b>', '0.07±0.01초', '376'],
     ['18×50 (300초)', 'lp', '100±0.0%', '4,224', '2.43±0.33초', '2,066'],
     ['', 'model', '100±0.0%', '<b>1,085</b>', '<b>1.82±0.37초</b>', '731'],
     ['21×60 (600초)', 'lp', '100±0.0%', '82,366', '51.5±2.0초', '1,627'],
     ['', 'model', '100±0.0%', '<b>8,657</b>', '<b>14.6±0.7초</b>', '619']],
    [3.0*cm, 2.2*cm, 2.4*cm, 2.8*cm, 3.0*cm, 1.8*cm],
    '표 7. warm start 루프에서의 branching 전략 비교, lattice 단계 비활성화, 각 칸 30개 instance, seed 3개의 seed별 중앙값에 대한 '
    '평균±표준편차. node 수는 seed에 불변이다.')
P('세 가지를 짚는다. <b>첫째</b>, 학습된 guidance는 모든 크기에서 더 작은 tree를 만들고 — lp 대비 node 2.0배, 3.9배, 9.5배 적음 — '
  '그 비율은 크기에 따라 커진다. §4.9의 분석이 예측하는 대로다: network가 relaxation rounding과 갈라지는 상태는 큰 tree의 위쪽에 '
  '있는 크고 약하게 제약된 상태이며, 10×25에는 그런 상태가 거의 없다. <b>둘째</b>, 작은 tree가 시간을 사는 것은 tree가 forward '
  'pass 비용을 상쇄할 만큼 커진 뒤부터다. 10×25에서 model arm은 오히려 <b>느리다</b>(0.34~0.43배, 30개 중 최대 1개에서 우세) — '
  '24-node tree는 그것을 줄이는 network 평가보다 싸기 때문이다. model의 node당 비용은 모든 크기에서 lp의 2.6~4.7배이며 '
  '이는 §4.5의 forward pass다. 18×50에서 인스턴스별 시간 비율 중앙값은 network 쪽으로 1.30~1.37배이지만 유의하지 않다'
  '(모든 seed에서 30개 중 18개 우세, 부호검정 <i>p</i>=0.36). <b>셋째</b>, 21×60에서 우위가 뚜렷하고 <b>재현된다</b>: '
...
TBL([['', 'CP-SAT', '제안', '비율'],
     ['node (중앙값)', '18,827 branches', '<b>8,657</b>', '우리가 2.2배 적음'],
     ['처리량', '<b>19,782 node/초</b>', '619 node/초', 'CP-SAT 32배 빠름'],
...
P('학습된 guidance는 CP-SAT보다 <b>작은 탐색 트리</b>를 만든다 — CP-SAT의 clause learning에도 불구하고 node가 '
  '2.2배 적다 — 그리고 전적으로 <b>node당 비용</b>에서 진다. 그 비용이 어디로 가는지는 측정할 수 있다. warm start 이전의 루프는 '
  '초당 254 node를 처리하며 node당 <b>4.34ms</b>를 relaxation 재구성·풀이에 썼다. 같은 LP를 warm start하면 그것이 <b>0.08ms</b>가 '
  '되고 처리량은 초당 619 node로 오르는데, 이것이 §4.6 전체에서 보고한 루프다. node당 남는 1.6ms 중 network forward pass가 '
  'root에서 <b>1.88ms</b>(축소 graph가 작아지는 깊은 node에서는 그보다 적음), propagation과 bookkeeping이 0.1ms 미만이다: '
  '이제 forward pass가 node당 비용의 90% 이상이며, 우리는 그것을 줄이지 않았다. 따라서 <b>격차가 구현 산물이라고 주장하지 '
  '않는다.</b> node 수가 확립하는 것은 더 좁다: 부족분이 branching 품질에 있지는 않다는 것이다. 격차를 닫으려면 최소한 compiled '
  'inference 경로와, 다음 단락이 적듯 우리 탐색에 없는 알고리즘 구성요소가 필요하다.', body_s)
P('이 해석에는 정직한 단서 두 가지가 붙는다. 트리 크기가 완전히 통약 가능하지는 않다 — CP-SAT의 "branch"에는 '
  'restart와 conflict 기반 점프가 포함되고 각 node가 우리보다 많은 propagation을 수행한다 — 따라서 2.2배는 '
  '자릿수 수준의 진술로 읽어야 한다. 그리고 경쟁력 있는 구현에는 빠른 forward pass 이상이 필요하다: '
  '<b>clause learning과 restart</b>가 우리 탐색에 없고, 뒤의 둘은 CP-SAT가 우리가 '
...
TBL([['학습 크기', '평가 크기', '해결율', '시간 중앙값', 'node 중앙값'],
    ...
    '표 10. warm start 루프에서의 transfer, lattice 단계 비활성화, 행마다 30개 instance, seed 3개. '
    '2~3행과 4~5행은 각각 평가 크기를 고정하고 학습 크기만 바꾼 것이다.')
P('21×60 평가 크기에서, 2.4배 작은 문제인 10×25에서만 학습한 network는 18×50에서 학습한 network보다 <b>못하지 않다</b>. '
  '굳이 말하면 낫다: 30개 중 18개에서 node가 더 적고 각 seed에서 18~19개에서 더 빨리 끝나며, 인스턴스별 시간 비율 중앙값은 '
  '그쪽으로 1.24~1.31배다. 어느 차이도 유의하지 않고(부호검정 <i>p</i>≥0.20) 둘 다 30/30을 푼다. 18×50에서도 같은 그림이다: '
  '10×25 network가 30개 중 16~17개에서 더 빠르지만(<i>p</i>≥0.59) 시간 중앙값은 약간 높은데, 짝지은 차이가 instance 간 변동에 비해 '
  '작아 두 요약이 엇갈리는 것이다. 재구성 루프에서의 이전 비교는 15/30과 0.92배를 주었다. 방향은 루프에 따라 안정적이지 않으며, '
  '우리는 이 결과를 transfer 이득이 아니라 <b>검출 가능한 transfer 손실 없음</b>으로 읽는다. root 수준 예측도 같은 결론을 준다: '
...
TBL([['크기', '전략', 'lattice가 해결', '잔여를 search가 해결', '잔여 시간 중앙값', 'end-to-end'],
    ...
    '뽑기 때문이며, 탐색에 넘어오는 잔여도 그에 따라 변한다. 한 seed 안에서 모든 arm은 동일한 잔여를 받는다.', font=8.0)
P('lattice coverage는 크기에 따라 떨어진다 — 30/30, 24~28/30, 10~16/30 — 그리고 그 감소가 <b>branching 품질이 '
  '중요해질 여지를 만든다.</b> 10×25와 18×50에서는 잔여가 비어 있거나 어느 건전 전략으로도 수 초 안에 닫힌다. '
  '21×60에서는 seed에 따라 10~20개 instance가 search로 넘어오고, 그 잔여는 진정으로 어렵다: random branching은 seed 합산 잔여 '
  '50개 중 하나도 닫지 못하므로 그 arm의 end-to-end 해결율은 lattice coverage 그 자체다. relaxation 기반과 학습된 branching은 '
  '둘 다 50개를 전부 닫으며, 둘의 차이는 <b>시간</b>이다. 잔여에서 학습된 guidance는 세 seed에서 14개 중 11개, 16개 중 14개, '
  '20개 중 15개에서 더 빠르고(인스턴스별 비율 중앙값 3.48배, 2.99배, 2.38배; 부호검정 <i>p</i>=0.057, 0.004, 0.041), '
