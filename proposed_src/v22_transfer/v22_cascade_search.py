"""v22: the full factorial -- guidance arms x {AHL off, AHL on} x transfer conditions.

v20 ran only the AHL-off ablation, which isolates branching quality but makes 40x100
unsolvable for every arm and so cannot speak to deployment. Reporting the ablation at
one size and the deployment setting at another would be indefensible, so this runs BOTH
settings at EVERY size and every arm; the AHL switch becomes an explicit variable rather
than a per-size choice.

  ahl=off : propagation + guided branching only. Isolates guidance quality.
  ahl=on  : the real cascade -- AHL first with a fixed share of the budget, then guided
            branching on whatever AHL failed to solve. AHL is guidance-independent, so
            every arm receives exactly the same residual subset; conditioning on it is a
            description of the algorithm, not a selection effect.

Both are reported together, including the case where the ablation collapses -- that
collapse quantifies how much the lattice stage carries at a given size.
"""
...
from v20_guided_search import GUIDES, search
...
def _worker(arm, ahl_on, inst_path, out_path, ckpt, time_limit, ahl_share, block, tries, seed_off=0):
	...
	if solved_by is None and budget > 0:
		ok, nodes, el = search(A, b, GUIDES[arm], model, device, budget, seed)
		if ok:
			solved_by = 'search'
	else:
		nodes, el = 0, 0.0
...
def main():
	...
	print("\n" + "=" * 92)
	print(f"v22 {a.tag}  (budget {a.time_limit}s, AHL block={a.block} tries={a.tries})")
	print("=" * 92)
